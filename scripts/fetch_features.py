#!/usr/bin/env python3
"""
Fetch the railway and the ponds around the course and bake them into
rttr_course_map.html as DATA.rail and DATA.ponds.

The Santa Cruz, Big Trees & Pacific runs out of Roaring Camp, crosses Pipeline
Road and trestles the San Lorenzo. Runners start beside it and cross its line,
so it belongs on an operations map. Bridge segments are tagged so the trestle
can be drawn heavier than plain track.

Ponds come from the USGS National Hydrography Dataset, same source as the
river.

    python3 scripts/fetch_features.py            # preview
    python3 scripts/fetch_features.py --write

Stdlib only.
"""

import argparse
import json
import math
import re
import sys
import urllib.parse
import urllib.request
from pathlib import Path

HTML = Path(__file__).resolve().parent.parent / "rttr_course_map.html"
OVERPASS = "https://overpass-api.de/api/interpreter"
NHD_WB = ("https://services.arcgis.com/P3ePLMYs2RVChkJx/arcgis/rest/services/"
          "NHDPlus_High_Resolution_9March2023_view/FeatureServer/1/query")
R_EARTH = 6378137.0

# Live track only. Abandoned and disused grades are historically interesting
# and completely invisible on the ground.
LIVE = {"rail", "narrow_gauge", "light_rail", "preserved"}


def read_track(text):
    m = re.search(r"\n  track: (\[\[.*?\]\]),\n", text, re.S)
    if not m:
        sys.exit("Could not find DATA.track in the HTML.")
    return json.loads(m.group(1))


def local_proj(lat0):
    kx = math.radians(1.0) * R_EARTH * math.cos(math.radians(lat0))
    ky = math.radians(1.0) * R_EARTH
    return lambda lat, lon: (lon * kx, lat * ky)


def seg_dist(px, py, ax, ay, bx, by):
    dx, dy = bx - ax, by - ay
    d2 = dx * dx + dy * dy
    if d2 <= 0:
        return math.hypot(px - ax, py - ay)
    t = max(0.0, min(1.0, ((px - ax) * dx + (py - ay) * dy) / d2))
    return math.hypot(px - (ax + t * dx), py - (ay + t * dy))


def simplify(pts, tol):
    if len(pts) < 3:
        return pts
    keep = [False] * len(pts)
    keep[0] = keep[-1] = True
    stack = [(0, len(pts) - 1)]
    while stack:
        s, e = stack.pop()
        ax, ay = pts[s]
        bx, by = pts[e]
        worst, idx = 0.0, -1
        for i in range(s + 1, e):
            d = seg_dist(pts[i][0], pts[i][1], ax, ay, bx, by)
            if d > worst:
                worst, idx = d, i
        if worst > tol and idx > 0:
            keep[idx] = True
            stack.append((s, idx))
            stack.append((idx, e))
    return [p for p, k in zip(pts, keep) if k]


def path_length(xy):
    return sum(math.dist(xy[i - 1], xy[i]) for i in range(1, len(xy)))


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--pad", type=float, default=220.0)
    ap.add_argument("--min-length", type=float, default=45.0)
    ap.add_argument("--tolerance", type=float, default=4.0)
    ap.add_argument("--write", action="store_true")
    args = ap.parse_args()

    text = HTML.read_text(encoding="utf-8")
    track = read_track(text)
    lat0 = sum(p[0] for p in track) / len(track)
    proj = local_proj(lat0)
    padlat = args.pad / 111320.0
    padlon = args.pad / (111320.0 * math.cos(math.radians(lat0)))
    bbox = (min(p[0] for p in track) - padlat, min(p[1] for p in track) - padlon,
            max(p[0] for p in track) + padlat, max(p[1] for p in track) + padlon)

    # ---- railway, from OSM ------------------------------------------------
    q = (f'[out:json][timeout:90];way["railway"]'
         f'({bbox[0]:.6f},{bbox[1]:.6f},{bbox[2]:.6f},{bbox[3]:.6f});out tags geom;')
    print("Querying OSM for railway ...")
    req = urllib.request.Request(OVERPASS, data=urllib.parse.urlencode({"data": q}).encode(),
                                 headers={"User-Agent": "rttr-course-map/1.0"})
    data = json.loads(urllib.request.urlopen(req, timeout=150).read().decode())

    rail = []
    for e in data.get("elements", []):
        t = e.get("tags") or {}
        if t.get("railway") not in LIVE or not e.get("geometry"):
            continue
        coords = [(g["lat"], g["lon"]) for g in e["geometry"]]
        xy = [proj(la, lo) for la, lo in coords]
        if path_length(xy) < args.min_length:
            continue
        simp = simplify(xy, args.tolerance)
        idx = {id(p): i for i, p in enumerate(xy)}
        kept = [coords[idx[id(p)]] for p in simp]
        rail.append({
            "name": t.get("name"),
            "bridge": bool(t.get("bridge")),
            "gauge": "narrow" if t.get("railway") == "narrow_gauge" else "standard",
            "path": [[round(a, 6), round(b, 6)] for a, b in kept],
            "len": path_length(xy),
        })
    rail.sort(key=lambda r: -r["len"])
    print(f"  {len(rail)} track segments, "
          f"{sum(1 for r in rail if r['bridge'])} of them bridges")
    for r in rail[:12]:
        print(f"    {r['gauge']:<8} {'BRIDGE ' if r['bridge'] else '       '}"
              f"{r['len']:6.0f} m  {r['name'] or '-'}")

    # ---- ponds, from USGS NHD --------------------------------------------
    params = {
        "geometry": f"{bbox[1]:.6f},{bbox[0]:.6f},{bbox[3]:.6f},{bbox[2]:.6f}",
        "geometryType": "esriGeometryEnvelope", "inSR": "4326", "outSR": "4326",
        "spatialRel": "esriSpatialRelIntersects",
        "outFields": "gnis_name,ftype,areasqkm", "returnGeometry": "true", "f": "json",
    }
    print("Querying USGS NHD for waterbodies ...")
    d2 = json.loads(urllib.request.urlopen(
        NHD_WB + "?" + urllib.parse.urlencode(params), timeout=120).read().decode())
    ponds = []
    for f in d2.get("features", []):
        a = f.get("attributes") or {}
        # 390 LakePond, 436 Reservoir. Everything else is swamp or wash.
        if a.get("ftype") not in (390, 436):
            continue
        for ring in (f.get("geometry") or {}).get("rings", []):
            coords = [(p[1], p[0]) for p in ring if len(p) >= 2]
            if len(coords) < 4:
                continue
            xy = [proj(la, lo) for la, lo in coords]
            simp = simplify(xy, args.tolerance / 2)
            idx = {id(p): i for i, p in enumerate(xy)}
            kept = [coords[idx[id(p)]] for p in simp]
            ponds.append({"name": a.get("gnis_name"),
                          "ring": [[round(la, 6), round(lo, 6)] for la, lo in kept],
                          "area": a.get("areasqkm") or 0})
    ponds.sort(key=lambda p: -p["area"])
    print(f"  {len(ponds)} ponds")
    for p_ in ponds:
        print(f"    {p_['area']*100:.2f} ha  {len(p_['ring']):3d} pts  {p_['name'] or '-'}")

    rail_src = ",\n".join(
        "    { name:%s, bridge:%s, gauge:%s, path:%s }" % (
            json.dumps(r["name"]) if r["name"] else "null",
            "true" if r["bridge"] else "false", json.dumps(r["gauge"]),
            "[" + ",".join("[%s,%s]" % (p[0], p[1]) for p in r["path"]) + "]")
        for r in rail)
    pond_src = ",\n".join(
        "    { name:%s, ring:%s }" % (
            json.dumps(p_["name"]) if p_["name"] else "null",
            "[" + ",".join("[%s,%s]" % (q[0], q[1]) for q in p_["ring"]) + "]")
        for p_ in ponds)

    if not args.write:
        print("\nPreview only. Re-run with --write to patch the HTML.")
        return

    new = text
    for key, src, note in [
        ("rail", rail_src,
         "  // The Santa Cruz, Big Trees & Pacific out of Roaring Camp. Runners start\n"
         "  // beside it and cross its line, so it belongs here. `bridge` marks the\n"
         "  // trestles, drawn heavier than plain track.\n"),
        ("ponds", pond_src,
         "  // Ponds at Roaring Camp, from the USGS National Hydrography Dataset.\n"),
    ]:
        if f"\n  {key}: [" in new:
            new = re.sub(rf"\n  {key}: \[.*?\n  \],",
                         f"\n  {key}: [\n{src}\n  ],", new, count=1, flags=re.S)
        else:
            new = new.replace("\n  // Optional: other park trails for context.",
                              f"\n{note}  {key}: [\n{src}\n  ],\n"
                              "\n  // Optional: other park trails for context.", 1)
    if new == text:
        sys.exit("Nothing was replaced -- the DATA block markers moved?")
    HTML.write_text(new, encoding="utf-8")
    print(f"\nPatched {HTML.name}: {len(rail)} track segments, {len(ponds)} ponds.")


if __name__ == "__main__":
    main()
